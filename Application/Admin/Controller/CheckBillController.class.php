...
class CheckBillController extends BaseController {
    ...
    public function billManage(){
        ...
        $responce->records = $count; //总记录数
       	...
       	$list = $model->table('cq_product a')->join('cq_plat b ON a.PLAT_SHORTNAME = b.PLAT_ID')->field('a.PRODUCT_ID,a.TARGET_NAME,b.PLAT_SHORTNAME,b.PLAT_ID')->where($where)->order($sidx.' '.$sord)->limit($start,$limit)->select();
    ...
    public function billRecord(){
        ...
        $count=$model->table('cq_product a')->join('left join cq_product_buy b ON a.PRODUCT_ID = b.PRODUCT_ID')->join('left join lc_user c ON b.USER_ID = c.USER_ID')->where($where)->count();
    	...
    	$list = $model->table('cq_product a')
    		...
    		->where($where)->order($sidx.' '.$sord)->limit($start,$limit)->select();
    	foreach ($list as $key => $value) {
    ...
    public function billCheck(){
        ...
        $what['a.BUY_MONEY']=array("gt",0);
        $what['a.IS_DEL']=0;
        $result=M("cq_product_buy a")->join("join cq_user_finance_record b on a.SERIAL_NO=b.SERIAL_NO")->where($what)->field("a.USER_ID,a.BUY_MONEY,b.USER_FINANCE_RECORE_ID,b.CASH_MONEY")->select();
    ...
    public function billChange(){
        ...
        $list = $model->field('BUY_MONEY,BUY_TIME')->where($where)->find();
    ...
    public function billXiugai(){
        ...
        //进行返利记录处理
          //查询产品相关信息
        $result=M("cq_product_buy a")->join("cq_product b on a.PRODUCT_ID=b.PRODUCT_ID")->where("a.PRODUCT_BUY_ID=$product_buy_id and a.IS_DEL=0")->field("a.USER_ID,a.BUY_MONEY,a.BUY_TIME,a.SERIAL_NO,b.TARGET_NAME,b.INVEST_MONTH,b.INVEST_DAY,b.CQ_RATE")->find();
            //修改
        if ($type_id==1) {
            //计算返利
            ...
            if ($result['invest_month'] > 0) {
                ...
                //之前的返利
            $finance_data=M("cq_user_finance_record")->where("USER_ID='".$result['user_id']."' and SERIAL_NO='".$result['serial_no']."' and IS_DEL=0")->field('CASH_MONEY')->find();
