...
class Page{
    ...
    private function url($page){
        /*$pageUrl = str_replace(strtolower(urlencode('[PAGE]')), $page, $this->url);
    ...
    //重写一下
        ...
        $this->url = U(ACTION_NAME, $this->parameter);
        if ($num) {
            $allurl = explode("/",$this->url);
            $this->url = "/".$allurl[1]."/".$allurl[3];
            for($i=0;$i < $num;$i++){
                $this->url.="/".$allurl[5+$i*2];
            }
        }
        /* 计算分页信息 */
        $this->totalPages = ceil($this->totalRows / $this->listRows); //总页数
